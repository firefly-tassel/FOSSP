    ...
    if not if_known:
        ...
        if len(semantic_center_map.keys()) == num_class:
            ...
            for recorded_unknown_class in recorded_unknowns:
                ...
                if dist <= (min_dist + coef_unknown * mean_dist) / (1 + coef_unknown):
                    if_recorded = True
                    break
    ...
    with open(zero_shot_path, 'w') as f:
        ...
        for coef_unknown in range(11):
            ...
            while coef <= 1.0:
                ...
                resultlines = []
                RESULT_LOGGER(resultlines, 'Distance {} with coefficient1 {}, coefficient2 {}'.format(distance, round(coef, 2), coef_unknown))
            ...
            sensity.append([tkr_list, tur_list, oa_list, f1_list])

        np.save(f'./results/{version}/sensity.npy', sensity)
...
if __name__ == '__main__':
    ...
    if not os.path.exists('results/' + version):
